          ...
          <div className="w-full  flex flex-col items-center justify-start gap-[5px]">
               <div className="w-full flex items-center justify-center relative h-auto">
                    ...
                    <div className="bg-white w-full aspect-[4/3] rounded-[5px] overflow-hidden flex items-center justify-center py-[2px] relative ">
                         <Image src={imageViewed.url} fill alt="Vehicle image" className={`w-full aspect-auto object-cover `} />
                    </div>
               ...
               <div className="w-full grid grid-cols-4 md:grid-cols-5 lg:grid-cols-6 2xl:grid-cols-8 gap-[5px] py-[5px]">
                    {
                         images.map((image, index) => <Image src={image.url} width={400} height={300} alt={"vehicle image"} key={`vehicle-other-image-${index}`} className={`w-full aspect-[4/3] rounded-[5px] cursor-pointer hover:border-[1.2px] hover:border-main-orange-400 transition-all duration-200 ${index === currentImage ? "border-[1.4px] border-gray-400 p-[2.5px] " : ""} `} onClick={() => setImageViewed(image)} />)
                    }
               </div>
          </div>
     )
