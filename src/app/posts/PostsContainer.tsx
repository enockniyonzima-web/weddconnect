...
const PostsContainer = ({posts}:{posts:TPost[]}) => {
     return (
          ...
          {
               posts.length > 0 ? 
               <div className='w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-[10px]'>
                    {posts.map((post, index) => <PostCard post={post} key={`posts-page-post-${index}`} />)}
               </div>
               :
