...
import { createPost, updatePost } from '@/server-actions/post.actions';
...
const AddPostForm = ({vendors,categories, post}: {vendors: TVendor[], categories: TCategory[], post: TPost | null}) => {
     ...
     if(selectedCategory === 0) return (
          <div className='w-full flex flex-col items-start justify-start gap-[10px] border rounded-[10px] p-[10px]'>
               <SelectInputGroup name='post category' label='Select Category:' values={categories.map(category => ({label:category.name, value: category.id}))} action={res => setSelectedCategory(Number(res))} />
