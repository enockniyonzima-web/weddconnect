...
import { createBlogTag, deleteBlogTag, fetchBlogTagById, updateBlogTag } from "@/server-actions/blog/blog-tag";
...
import { EditIcon, PlusIcon, Tag, X } from "lucide-react";
import { useState } from "react";
import { Dialog, DialogPanel } from "@headlessui/react";
import { cn } from "@/lib/utils";

export const BlogTagForm = ({id, onComplete}:{id?:string, onComplete: () => void}) => {
     const {data: TagData, isLoading} = useQuery({
          queryKey: ["blog-Tag-form", id],
          queryFn: () => fetchBlogTagById(id!, {id: true, name: true}),
          enabled: !!id
     });
     const Tag = TagData ?? null;
     const submitData = async (data: FormData): Promise<{message?: string}> => {
          ...
          const updatedTag = await updateBlogTag(id, {
               ...(name && {name, slug}),
          })
          if(!updatedTag) throw new Error("Error updating the Tag");
          onComplete();
          queryClient.invalidateQueries();
     ...
     const handleDelete = async () => {
          toast.promise(
               (async () => {
                    if(!Tag) throw new Error("The Tag does not exist in the system!");
                    const res = await deleteBlogTag(Tag.id);
                    if(!res) throw new Error("Server Error deleting the Tag!");
                    onComplete();
                    await queryClient.invalidateQueries();
               })(),
               {
                    loading: "Deleting blog Tag...",
                    success: "Tag Deleted Successfully",
                    error: error => error.message ?? "Failed to delete the Tag"
               }
          )
     ...
     return (
          <div className="w-full flex flex-col gap-4">
               <MainForm submitData={submitData} btnTitle={Tag ? "Update Tag" : "Create Tag"}>
                    <TextInput name="name" defaultValue={Tag?.name} placeholder="ex Technology" required={Tag ? false: true} label="Tag Name:" />
               </MainForm>
               {id && <DeleteBtn onClick={handleDelete} title="Delete Tag" confirmMessage="Are you sure you want to delete this Tag?" confirmTitle="Delete" />}
          </div>
     )
}

export const BlogTagFormBtn = ({TagId, showBtnName, showBtnIcon, btnSize = "md"}:{TagId?: string, showBtnName?: boolean; showBtnIcon?: boolean; btnSize?: InputSize}) => {
     const btnTitle = showBtnName ? TagId ? "Edit" : "Create" : undefined;
     const Icon = TagId ? EditIcon : PlusIcon;
     ...
     return (
          <>
               <button type="button" onClick={() => setOpen(true)} className={cn("inline-flex items-center justify-center rounded-lg font-medium cursor-pointer transition-all duration-200 active:scale-[0.97]", sizeClasses[btnSize], TagId ? "border border-white/10 bg-blue-700 text-gray-100 hover:border-blue-700/30 hover:text-white" : "bg-blue-600 text-white shadow-lg shadow-blue-600/20 hover:bg-blue-500")}>
                         ...
                         <DialogPanel className="relative w-[92vw] max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 bg-gray-900 shadow-2xl shadow-black/50">
                              ...
                              <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-white/5 bg-gray-900/95 backdrop-blur-md px-6 py-4 rounded-t-2xl">
                                   <div className="flex items-center gap-3">
                                        <div className={cn("flex items-center justify-center h-9 w-9 rounded-lg", TagId ? "bg-blue-600/10 border border-blue-600/20 text-blue-600" : "bg-blue-600 text-white shadow-md shadow-blue-600/25")}><Tag size={18} strokeWidth={2} /></div>
                                        <div><h3 className="text-base font-semibold text-white">{TagId ? "Edit Tag" : "Create Tag"}</h3><p className="text-xs text-gray-400">{TagId ? "Update Tag details" : "Create a new Tag"}</p></div>
                                   </div>
                              ...
                              <div className="relative px-6 py-5"><BlogTagForm id={TagId} onComplete={() => setOpen(false)} /></div>
