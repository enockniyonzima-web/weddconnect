...
export const ListSection = ({content} :{content: {title:string, pos: number,paragraphs:string[], intro?:string, type:string}}) => {
     return (
          <section className="w-full flex flex-col items-start justify-start gap-[20px]">
               <h2 className="text-[1.2rem] font-bold text-black">{content.pos}. {content.title}</h2>
               <div className="w-full flex flex-col gap-[5px]">
                    {content.intro && <p className="text-[1rem] text-gray-800">{content.intro}</p>}
               </div>
