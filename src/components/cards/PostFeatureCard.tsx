...
export const PostFeatureCard = ({feature}: {feature:TPostFeature}) => {
     ...
     return(
          <div title={feature.categoryFeature.name} className="w-auto cursor-pointer flex flex-col justify-start items-center gap-[3px] rounded-lg p-[2.5px] py-[5px] border border-gray-800 bg-gray-900">
               <p className='text-[0.65rem] text-gray-400 font-semibold'>{feature.categoryFeature.name}</p>
               <div className="w-auto max-w-[100%] flex items-center gap-[2px] overflow-hidden">
                    {Icon && <i className="text-blue-400 text-[18px] md:text-[18px] p-[1px]"><Icon /></i>}
                    <span className="text-[0.85rem] lg:text-[0.7rem] text-white font-semibold whitespace-nowrap w-[70%] line-clamp-1">: {feature.categoryFeature.name.toLowerCase() === "languages"? feature.value.trim().split(",").length : feature.value}</span>
               </div>
          </div>
     )
}
